...
const OPERATIONS_BY_TPNF: Record<string, { value: string; label: string; icon: string }[]> = {
  '1': [ // SAÍDA — CFOPs 5xxx, 6xxx, 7xxx
    ...
    { value: 'devolucao', label: 'Devolução ao Fornecedor', icon: '' },
    ...
    return (
      <div>
        ...
        <div className="card">
          ...
          <div className="alert alert-warning mt-8">Ambiente de homologação — NF sem valor fiscal</div>
  ...
  return (
    <div>
          ...
          <div style={{ background: 'var(--bg)', borderRadius: 8, padding: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                ...
                {selectedCustomer.email && <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{selectedCustomer.email}</div>}
              </div>
      ...
      <div className="card">
        ...
        {fiscalResult?.cfop && (
          <div style={{ background: 'var(--bg)', borderRadius: 8, padding: 12, marginTop: 8 }}>
            ...
            {fiscalResult.mensagemAlerta && <div className="alert alert-warning mt-8" style={{ fontSize: 12 }}>{fiscalResult.mensagemAlerta}</div>}
          </div>
        )}
      </div>
        ...
        {items.map((item, idx) => (
          <div key={item.id} style={{ background: 'var(--bg)', borderRadius: 8, padding: 12, marginBottom: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
              ...
              {items.length > 1 && (
                <button onClick={() => setItems(prev => prev.filter(i => i.id !== item.id))} style={{ background: 'none', border: 'none', color: 'var(--danger)', cursor: 'pointer', fontSize: 16 }}>X</button>
              )}
            </div>
      ...
      <div className="alert alert-warning" style={{ fontSize: 12 }}>
        NF será gerada em <strong>homologação</strong> (sem valor fiscal)
      </div>
